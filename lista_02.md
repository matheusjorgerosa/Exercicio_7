...
var idade = digite('Insira sua idade');

se (idade for menor que 16) então:
	imprimir('Não pode votar')
    fimSE

se nao se (idade for maior ou igual 16 e menor que 18) então:
	imprimir('Voto facultativo!')
    fimSE

senao
	imprimir('Voto obrigatório!') então:
    fimSENÃO
...
Classe Retangulo herda FormaGeometrica:
    ...
    Método ConstrutorRetangulo(cor):
        Define o atributo cor com o valor passado como parâmetro.
    fim Método

    Método CalcularAreaRetangulo(largura, altura):
        areaRetangulo = largura*altura
        Retornar areaRetangulo
    fim Método
...
Classe Circulo herda FormaGeometrica:
    ...
    Método ContrutorCirculo(cor):
        Define o atributo cor com o valor passado como parâmetro.
    fim Método

    Método CalcularAreaCirculo(raio):
...
Enquanto (distanciaPercorrida < distancia) E (velocidadeAtual < velocidadeMaxima) E (tempo < tempoMaximo):
    velocidadeAtual = velocidadeInicial + aceleracao * tempo
    distanciaPercorrida += velocidadeAtual * tempo
    tempoFaltando = tempo + (distancia - distanciaPercorrida) / velocidadeAtual
fim Enquanto

Se (velocidadeAtual >= velocidadeMaxima):
    aceleracao -= 1
    fim Se
se (tempoFaltando >= tempoMaximo):
    aceleracao += 1
    fim Se
...
Função MultiplicacaoDeMatrizes(matrizA, matrizB):
    ...
    Para i de 0 até linha:
        Para j de 0 até linha:
            Para k de 0 até linha:
                matrizResultante[i][j] += matrizA[i][k] * matriz[k][j]
            fim Para
        fim Para
    fim Para
